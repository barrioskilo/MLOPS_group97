        ...
        if file.endswith(".pt"):
            ...
            for img in data_tensor:
                assert img.shape == (
                    ...
                    600,
                    600,
                ), f"Test failed: Each image should have shape (3, 600, 600), found {img.shape}"
            print("Test passed: All images have the correct shape.")

            # Check if the number of labels matches the number of images
            assert len(labels_tensor) == len(
                data_tensor
            ), "Test failed: Number of labels should match the number of images"
